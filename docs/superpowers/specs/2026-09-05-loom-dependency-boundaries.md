...
状态：历史设计依据，已形成[现行依赖规范](../../DEPENDENCY_MODEL.md)及 v1alpha2 静态 Schema/测试。本文保留讨论时的表述；查阅最终字段和迁移结果请使用现行规范。图为 Markdown Mermaid，不修改 Draw.io。
...
以上字段名为拟定契约。任务不从 components 容器自动获得全部能力；省略选择必须报错，不得创建全能默认岗位。自由对话且无 tasks 的旧配置继续使用其已声明的入口与能力视图，不作为新任务模式的隐式回退。
...
当前 v0.4 仍允许 assembly 装入 workflow 等组件，且 workflow.seat、tool.phase 存在跨层绑定；本方案不声称当前 Schema 已满足上述规则。
...
旧写法与新写法使用明确版本入口或显式迁移模式，不在同一模式下悄悄兼容两种解释。旧正例通过旧契约测试，新正例通过新契约测试；禁止为了让旧文件通过而放宽新分层规则。正式实施前必须确定机器可区分的版本方案，不能仅靠文档页眉区分结构性破坏变更。
